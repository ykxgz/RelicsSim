#include "RelicsOpticalPhysics.hh"
...
PhysicsRegister<RelicsOpticalPhysics>
    RelicsOpticalPhysics::reg("RelicsOpticalPhysics");

RelicsOpticalPhysics::RelicsOpticalPhysics(const BambooParameters &pars)
    : BambooPhysics(pars) {
    ...
    parameters->SetWLSTimeProfile("delta");

    parameters->SetCerenkovMaxPhotonsPerStep(100);
    parameters->SetCerenkovMaxBetaChange(10.0);

    parameters->SetScintTrackSecondariesFirst(true);
    parameters->SetCerenkovTrackSecondariesFirst(true);
#else
    // Optical Physics
